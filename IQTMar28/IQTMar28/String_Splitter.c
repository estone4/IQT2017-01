...
int main(void)
{
	...
	*/
	...
	char outputString3[] = { " character to determine the address of something." }; // Delimiter = &
...
char * split_the_string(char * string_ptr, char delimiter)
{
	...
	if (!delimiter)		// Check if the delimiter exists
	{
		new_ptr = ERROR_NULL_DELIMITER;
	}
	else if (!string_ptr)  // Check if the string pointer exists
	{
		new_ptr = ERROR_NULL_POINTER;
	}
	else
	{
		i = 0;
		while (*(string_ptr + i))
		{
			if (*(string_ptr + i) == delimiter)		// Check if the string point matches the delimiter
			{
				delimiterCount++;					// Count the delimiters
			}
			i++;									// Increment the index of the string pointer
		}
		if (delimiterCount > 1)						// Check if there are more than 1 delimiter
		{
			new_ptr = ERROR_ABUNDANT_DELIMITER;
		}
		else if (!delimiterCount)					// Check if the delimiter count is set
		{
			new_ptr = string_ptr;					// Set the new pointer to the old pointer if no delimiters are found
		}
		else
		{
			i = 0;
			do
			{
				...
				{
					(*(string_ptr + i) = 0);
					new_ptr = (string_ptr + i + 1);			// Set the new pointer 
				}
				i++;
			} while (!new_ptr);
		}
