...
struct MyStruct
{
	int testNumber;				// Holds the test number
	int startingNumber;			// Holds the starting number input for counting array
	int endingNumber;			// Holds the ending number input for counting array
	int expErrorCode;			// Holds the expected error code for counting array
	int expSize;				// Holds the expected dimensions
	int expResult;			// Holds the exoected return value from the counting array
};

int main(void)
{
	int i = 0;					// Iterating var
	int j = 0;					// Iterating var
	struct MyStruct input1;		// Test 1
	struct MyStruct input2;		// Test 2
	struct MyStruct input3;		// Test 3
	struct MyStruct input4;		// Test 4
	int returnValue = NULL;
	int errorCode = 0;
	int numTests = 0;			// Number of tests run
	int inputValue = 0;

	int expOutput[75 - 10 + 1] = { 0 };
	input1.startingNumber = 10;
	input1.endingNumber = 75;
	input1.testNumber = 1;
	input1.expErrorCode = 0;
	input1.expSize = (75 - 10 + 1);
	input1.expResult = expOutput;
	for (i = 0; i < input1.expSize; i++)
	{
		*(expOutput + i) = (input1.startingNumber + i);
	}

	struct MyStruct * testArray[] = { &input1 };

	for (i = 0; i < sizeof(testArray) / sizeof(*testArray); i++)
	{
		printf("INPUT #%d", (*(testArray + i))->testNumber);
		returnValue = counting_array((*(testArray + i))->startingNumber, (*(testArray+i))->endingNumber, &errorCode);

		// Array Test
		numTests++;
		printf("Array Results \n");

		inputValue = (*(testArray + i))->expResult;
		for (j = 0; j < (*(testArray + i))->expSize; j++)
		{
