...
int reverse_it(char * forwardString, int strLen)
{
	...
	if (!forwardString) // Checks for NULL pointer
	{
		return ERR_NULL_POINTER;
	}
	
	
	if (strLen <=0) // Checks for unrealistic input
	{
		return ERR_INVALID_LENGTH;
	}
	...
	/*
	*  PRINTs THE String beginning with the last character
	*/
		
	for (int i = 1; i <= strLen; i++)
	{
		printf("%c",(forwardString[strLen - i]));
	}
